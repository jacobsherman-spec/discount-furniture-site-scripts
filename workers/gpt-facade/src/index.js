...
async function internalFetch(env, method, path, body, query) {
  if (!env.INTERNAL_BRIDGE_API_KEY) {
    return { ok: false, status: 500, data: { error: "Server misconfigured: missing INTERNAL_BRIDGE_API_KEY" } };
  }

  if (!env.INTERNAL_BRIDGE_SERVICE && !env.INTERNAL_BRIDGE_URL) {
    return { ok: false, status: 500, data: { error: "Server misconfigured: missing INTERNAL_BRIDGE_SERVICE and INTERNAL_BRIDGE_URL" } };
  }

  const url = new URL(path, env.INTERNAL_BRIDGE_URL || "https://internal-bridge.invalid");
  if (query) {
  ...
  try {
    ...
    if (env.INTERNAL_BRIDGE_SERVICE) {
      const serviceReq = new Request(url.toString(), requestInit);
      res = await env.INTERNAL_BRIDGE_SERVICE.fetch(serviceReq);
    } else {
...
async function handleRead(body, env) {
  ...
  switch (action) {
    case "searchProducts":
      return bridgeResponse(await internalFetch(env, "GET", "/products/search", null, {
    ...
    case "getProductById": {
      ...
      return bridgeResponse(await internalFetch(env, "GET", `/products/${encodeURIComponent(productId)}`));
    }
    case "getProductInventory": {
      const productId = getField(body, "productId", "product_id");
      if (!productId) return jsonResponse({ error: "Missing required field: productId" }, 400);
      return bridgeResponse(await internalFetch(env, "GET", `/products/${encodeURIComponent(productId)}/inventory`));
    }
    case "getProductInventoryLevels": {
      const productId = getField(body, "productId", "product_id");
      if (!productId) return jsonResponse({ error: "Missing required field: productId" }, 400);
      return bridgeResponse(await internalFetch(env, "GET", `/products/${encodeURIComponent(productId)}/inventory-levels`));
    }
    case "listBrands": return bridgeResponse(await internalFetch(env, "GET", "/brands"));
    case "listSuppliers": return bridgeResponse(await internalFetch(env, "GET", "/suppliers"));
    case "listOutlets": return bridgeResponse(await internalFetch(env, "GET", "/outlets"));
    case "listGitHubBrandTemplates": return bridgeResponse(await internalFetch(env, "GET", "/templates/brands"));
    case "getGitHubBrandTemplate":
      return bridgeResponse(await internalFetch(env, "GET", "/templates/brand", null, { brand: getField(body, "brand", "brandName", "brand_name") }));
    case "getGitHubTemplateRegistry": return bridgeResponse(await internalFetch(env, "GET", "/templates/registry"));
    case "getGitHubTemplateReadme": return bridgeResponse(await internalFetch(env, "GET", "/templates/readme"));
    case "listGitHubBrandAssets": return bridgeResponse(await internalFetch(env, "GET", "/templates/assets"));
    case "getGitHubTemplateFile":
      return bridgeResponse(await internalFetch(env, "GET", "/templates/file", null, { path: getField(body, "path", "filePath", "file_path") }));
    case "catalogReport": {
      ...
      return bridgeResponse(await internalFetch(env, "GET", `/reports/${encodeURIComponent(reportType)}`));
    }
    case "auditHealth": return bridgeResponse(await internalFetch(env, "GET", "/audit/health"));
    case "listDescriptionUpdates": return bridgeResponse(await internalFetch(env, "GET", "/description-updates"));
    case "getDescriptionUpdate": {
      ...
      return bridgeResponse(await internalFetch(env, "GET", `/description-updates/${encodeURIComponent(historyId)}`));
    }
    case "getProductDescriptionHistory": {
      const productId = getField(body, "productId", "product_id");
      if (!productId) return jsonResponse({ error: "Missing required field: productId" }, 400);
      return bridgeResponse(await internalFetch(env, "GET", `/products/${encodeURIComponent(productId)}/description/history`));
    }
    default:
...
async function handlePreview(body, env) {
  ...
  if (type === "description_update") {
    ...
    return bridgeResponse(await internalFetch(env, "POST", `/products/${encodeURIComponent(productId)}/description/preview`, payload));
  }
  ...
  if (type === "description_rollback") {
    ...
    return bridgeResponse(await internalFetch(env, "POST", `/products/${encodeURIComponent(productId)}/description/rollback/preview`, payload));
  }
  ...
  if (type === "pricing_update") {
    ...
    const productRes = await internalFetch(env, "GET", `/products/${encodeURIComponent(productId)}`);
...
async function handleWrite(body, env) {
  ...
  if (type === "description_update") {
    ...
    return bridgeResponse(await internalFetch(env, "PUT", `/products/${encodeURIComponent(productId)}/description`, payload));
  }
  ...
  if (type === "description_rollback") {
    ...
    return bridgeResponse(await internalFetch(env, "PUT", `/products/${encodeURIComponent(productId)}/description/rollback`, payload));
  }
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && path === "/health") {
      const internal = await internalFetch(env, "GET", "/health");
      return jsonResponse({
        status: "ok",
